...
function Salutation(props) {
	...
	const { saluId } = routeParams;
	const [disable, setDisable] = useState(false);

	useDeepCompareEffect(() => {
		async function updateProductState() {
			console.log("saluId", saluId);
			if (saluId === 'new') {

			} else {
				let apiObj = await masterApi.getSalutationById(saluId);
				let obj = {
					id: apiObj.saluId,
					saluName: apiObj.saluName,
	...
	function onSubmit(values) {
		...
		if (values.button.toUpperCase() === "SAVE") {
			...
			lrData.saluName = values.data.saluName;
			lrData.status = values.data.status.toUpperCase();

			console.log(lrData);
			if (saluId === "new") {
				masterApi.saveToMaster({ name: 'salutation', formData: lrData })
			}
			else {
				masterApi.updateToMaster({ name: 'salutation', id: saluId, formData: lrData })
			}
		}
	...
	function onCancel() {
		//console.log(values);
		props.history.push("/app/master/salutation/listing");
	}
						...
						<div className="flex flex-col items-start max-w-full">
							<FuseAnimate animation="transition.slideRightIn" delay={300}>
								<Typography
									...
									component={Link}
									role="button"
									to="/app/master/salutation/listing"
									color="inherit"
								>
							...
							<div className="flex items-center max-w-full">
								...
								<div className="flex flex-col min-w-0 mx-8 sm:mc-16">
									...
									<FuseAnimate animation="transition.slideLeftIn" delay={300}>
										<Typography variant="caption"><Label labelId="BL00276" /></Typography>
									</FuseAnimate>
								</div>
							</div>
						</div>
