...
function Division(props) {
	...
	useDeepCompareEffect(() => {
		async function updateProductState() {
			...
			if (divId === 'new') {
				...
				let apiObj = await masterApi.getdivisionById(divId);
				let obj = {
					id: apiObj.divId,
					divName: apiObj.divName,
	...
	function onSubmit(values) {
		...
		if (values.button.toUpperCase() === "SAVE") {
			...
			lrData.divName = values.data.divName;
			lrData.status = values.data.status.toUpperCase();

			console.log(lrData);
			if (divId === "new") {
				masterApi.saveToMaster({ name: 'division', formData: lrData })
			}
			else {
				masterApi.updateToMaster({ name: 'division', id: divId, formData: lrData })
			}
		}
	...
	function onCancel() {
		//console.log(values);
		props.history.push("/app/master/division/listing");
	}
						...
						<div className="flex flex-col items-start max-w-full">
							<FuseAnimate animation="transition.slideRightIn" delay={300}>
								<Typography
									...
									component={Link}
									role="button"
									to="/app/master/division/listing"
									color="inherit"
								>
							...
							<div className="flex items-center max-w-full">
								...
								<div className="flex flex-col min-w-0 mx-8 sm:mc-16">
									...
									<FuseAnimate animation="transition.slideLeftIn" delay={300}>
										<Typography variant="caption"><Label labelId="BL00327" /></Typography>
									</FuseAnimate>
								</div>
							</div>
						</div>
