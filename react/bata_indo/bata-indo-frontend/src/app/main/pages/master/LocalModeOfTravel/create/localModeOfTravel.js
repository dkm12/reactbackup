...
function LocalModeOfTravel(props) {
	...
	const { lmotId } = routeParams;

	useDeepCompareEffect(() => {
		async function updateProductState() {
			console.log("lmotId", lmotId);
			if (lmotId === 'new') {

			} else {
				let apiObj = await masterApi.getLocModeOfTravelById(lmotId);
				let obj = {
					id: apiObj.lmotId,
					lmotName: apiObj.lmotName,
		...
		sections: [
			{
				...
				fields: [{
					...
					id: 'lmotName',
					name: 'lmotName',
					title: 'Local Mode Of Travel Name'
				}, {
					type: 'radio',
					id: 'status',
					name: 'status',
	...
	function onSubmit(values) {
		...
		if (values.button.toUpperCase() === "SAVE") {
			...
			lrData.lmotName = values.data.lmotName;
			lrData.status = values.data.status.toUpperCase();

			console.log(lrData);

			if (lmotId === "new") {
				masterApi.saveToMaster({ name: 'local-mode-of-travel', formData: lrData })
			}
			else {
				masterApi.updateToMaster({ name: 'local-mode-of-travel', id: lmotId, formData: lrData })
			}
	...
	function onCancel() {
		//console.log(values);
		props.history.push("/app/master/localmodeoftravel/create");
	}
						...
						<div className="flex flex-col items-start max-w-full">
							<FuseAnimate animation="transition.slideRightIn" delay={300}>
								<Typography
									...
									component={Link}
									role="button"
									to="/employee-service/travel-claim"
									color="inherit"
								>
							...
							<div className="flex items-center max-w-full">
								...
								<div className="flex flex-col min-w-0 mx-8 sm:mc-16">
									...
									<FuseAnimate animation="transition.slideLeftIn" delay={300}>
										<Typography variant="caption">Local Mode Of Travel</Typography>
									</FuseAnimate>
								</div>
							</div>
						</div>
