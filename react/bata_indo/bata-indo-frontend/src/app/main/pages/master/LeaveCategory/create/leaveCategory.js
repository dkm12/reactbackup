...
function LeaveCategory(props) {
	...
	const { leaveCategoryId } = routeParams;
	const [disable, setDisable] = useState(false);

	useDeepCompareEffect(() => {
		async function updateProductState() {
			console.log("leaveCategoryId", leaveCategoryId);
			if (leaveCategoryId === 'new') {

			} else {
				let apiObj = await masterApi.getleaveCategoryById(leaveCategoryId);
				let obj = {
					id: apiObj.lcId,
					lcName: apiObj.lcName,
	...
	function onSubmit(values) {
		...
		if (values.button.toUpperCase() === "SAVE") {
			...
			lrData.lcName = values.data.lcName;
			lrData.status = values.data.status.toUpperCase();

			console.log(lrData);
			if (leaveCategoryId === "new") {
				masterApi.saveToMaster({ name: 'leave-category', formData: lrData })
			}
			else {
				masterApi.updateToMaster({ name: 'leave-category', id: leaveCategoryId, formData: lrData })
			}
	...
	function onCancel() {
		//console.log(values);
		props.history.push("/app/master/leave-category/listing");
	}
		...
		<FusePageSimple
						...
						<div className="flex flex-col items-start max-w-full">
							<FuseAnimate animation="transition.slideRightIn" delay={300}>
								<Typography
									...
									component={Link}
									role="button"
									to="/app/master/leave-category/listing"
									color="inherit"
								>
							...
							<div className="flex items-center max-w-full">
								...
								<div className="flex flex-col min-w-0 mx-8 sm:mc-16">
									...
									<FuseAnimate animation="transition.slideLeftIn" delay={300}>
										<Typography variant="caption"><Label labelId="BL00170" /></Typography>
									</FuseAnimate>
								</div>
							</div>
						</div>
			...
			content={
				...
				<div className="p-16 sm:p-24">
					...
					{leaveCategoryId !== "new" && data && data.id && (
						<SmartForm
							defaultValues={data}
							template={template}
							//watchFields={['']}
