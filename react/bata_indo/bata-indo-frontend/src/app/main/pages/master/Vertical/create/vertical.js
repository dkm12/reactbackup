...
function Vertical(props) {
	...
	useDeepCompareEffect(() => {
		async function updateProductState() {
			...
			if (vtId === 'new') {
				...
				let apiObj = await masterApi.getverticalById(vtId);
				let obj = {
					id: apiObj.vtId,
					vtName: apiObj.vtName,
	...
	function onSubmit(values) {
		...
		if (values.button.toUpperCase() === "SAVE") {
			...
			lrData.vtName = values.data.vtName;
			lrData.status = values.data.status.toUpperCase();

			console.log(lrData);

			if (vtId === "new") {
				masterApi.saveToMaster({ name: 'vertical', formData: lrData })
			}
			else {
				masterApi.updateToMaster({ name: 'vertical', id: vtId, formData: lrData })
			}
	...
	function onCancel() {
		//console.log(values);
		props.history.push("/app/master/vertical/listing");
	}
						...
						<div className="flex flex-col items-start max-w-full">
							<FuseAnimate animation="transition.slideRightIn" delay={300}>
								<Typography
									...
									component={Link}
									role="button"
									to="/app/master/vertical/listing"
									color="inherit"
								>
							...
							<div className="flex items-center max-w-full">
								...
								<div className="flex flex-col min-w-0 mx-8 sm:mc-16">
									...
									<FuseAnimate animation="transition.slideLeftIn" delay={300}>
										<Typography variant="caption"><Label labelId="BL00359" /></Typography>
									</FuseAnimate>
								</div>
							</div>
						</div>
