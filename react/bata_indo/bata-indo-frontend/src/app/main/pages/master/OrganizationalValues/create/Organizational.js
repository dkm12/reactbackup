...
function Zone(props) {
	...
	useDeepCompareEffect(() => {
		async function updateProductState() {
			...
			if (orgValId === 'new') {
				...
				let apiObj = await masterApi.getOrganizationalById(orgValId);
				let obj = {
					id: apiObj.orgValId,
					orgValName: apiObj.orgValName,
	...
	function onSubmit(values) {
		...
		if (values.button.toUpperCase() === "SAVE") {
			...
			lrData.orgValName = values.data.orgValName;
			lrData.status = values.data.status.toUpperCase();

			console.log(lrData);
			if (orgValId === "new") {
				masterApi.saveToMaster({ name: 'organizational-values', formData: lrData })
			}
			else {
				masterApi.updateToMaster({ name: 'organizational-values', id: orgValId, formData: lrData })
			}
	...
	function onCancel() {
		//console.log(values);
		props.history.push("/app/master/organizational-values/listing");
	}
						...
						<div className="flex flex-col items-start max-w-full">
							<FuseAnimate animation="transition.slideRightIn" delay={300}>
								<Typography
									...
									component={Link}
									role="button"
									to="/app/master/organizational-values/listing"
									color="inherit"
								>
							...
							<div className="flex items-center max-w-full">
								...
								<div className="flex flex-col min-w-0 mx-8 sm:mc-16">
									...
									<FuseAnimate animation="transition.slideLeftIn" delay={300}>
										<Typography variant="caption"><Label labelId="BL00229" /></Typography>
									</FuseAnimate>
								</div>
							</div>
						</div>
