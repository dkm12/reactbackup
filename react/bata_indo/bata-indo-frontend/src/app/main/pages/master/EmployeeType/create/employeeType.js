...
function EmployeeType(props) {
	...
	const { employeeTypeId } = routeParams;
	const [disable, setDisable] = useState(false);

	useDeepCompareEffect(() => {
		async function updateProductState() {
			console.log("employeeTypeId", employeeTypeId);
			if (employeeTypeId === 'new') {

			} else {
				let apiObj = await masterApi.getEmployeeTypeById(employeeTypeId);
				let obj = {
					id: apiObj.etId,
					etName: apiObj.etName,
	...
	function onSubmit(values) {
		...
		if (values.button.toUpperCase() === "SAVE") {
			...
			lrData.etName = values.data.etName;
			lrData.status = values.data.status.toUpperCase();

			console.log(lrData);

			if (employeeTypeId === "new") {
				masterApi.saveToMaster({ name: 'employee-type', formData: lrData })
			}
			else {
				masterApi.updateToMaster({ name: 'employee-type', id: employeeTypeId, formData: lrData })
			}
		}
	...
	function onCancel() {
		//console.log(values);
		props.history.push("/app/master/employee-type/listing");
	}
		...
		<FusePageSimple
						...
						<div className="flex flex-col items-start max-w-full">
							<FuseAnimate animation="transition.slideRightIn" delay={300}>
								<Typography
									...
									component={Link}
									role="button"
									to="/app/master/employee-type/listing"
									color="inherit"
								>
							...
							<div className="flex items-center max-w-full">
								...
								<div className="flex flex-col min-w-0 mx-8 sm:mc-16">
									...
									<FuseAnimate animation="transition.slideLeftIn" delay={300}>
										<Typography variant="caption"><Label labelId="BL00331" /></Typography>
									</FuseAnimate>
								</div>
							</div>
						</div>
			...
			content={
				...
				<div className="p-16 sm:p-24">
					...
					{employeeTypeId !== "new" && data && data.id && (
						<SmartForm
							defaultValues={data}
							template={template}
							//watchFields={['']}
