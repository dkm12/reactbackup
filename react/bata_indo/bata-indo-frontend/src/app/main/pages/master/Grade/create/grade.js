...
function Grade(props) {
	...
	useDeepCompareEffect(() => {
		async function updateProductState() {
			...
			if (gradeId === 'new') {
				...
				let apiObj = await masterApi.getGradeById(gradeId);
				let obj = {
					id: apiObj.gradeId,
	...
	function onSubmit(values) {
		...
		if (values.button.toUpperCase() === "SAVE") {
			...
			lrData.gradeName = values.data.gradeName;
			lrData.status = values.data.status.toUpperCase();

			console.log(lrData);

			if (gradeId === "new") {
				masterApi.saveToMaster({ name: 'grade', formData: lrData })
			}
			else {
				masterApi.updateToMaster({ name: 'grade', id: gradeId, formData: lrData })
			}
	...
	function onCancel() {
		//console.log(values);
		props.history.push("/app/master/grade/listing");
	}
						...
						<div className="flex flex-col items-start max-w-full">
							...
							<div className="flex items-center max-w-full">
								...
								<div className="flex flex-col min-w-0 mx-8 sm:mc-16">
									...
									<FuseAnimate animation="transition.slideLeftIn" delay={300}>
										<Typography variant="caption"><Label labelId="BL00337" /></Typography>
									</FuseAnimate>
								</div>
							</div>
						</div>
