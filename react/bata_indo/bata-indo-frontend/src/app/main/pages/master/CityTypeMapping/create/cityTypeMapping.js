...
function CityTypeMapping(props) {
	...
	const { cityTpeMappingId } = routeParams;
	const [disable, setDisable] = useState(false);
	useDeepCompareEffect(() => {
		async function updateProductState() {
			console.log("cityTpeMappingId", cityTpeMappingId);
			if (cityTpeMappingId === 'new') {

			} else {
				let data = await masterApi.getAllCityTypeMappingById(cityTpeMappingId);
				console.log("cityObj", data);
				let obj = {
					id: data.ctmId,
					ctmCityType: data.ctmCityType,
	...
	function onSubmit(values) {
		...
		if (values.button.toUpperCase() === "SAVE") {
			...
			lrData.ctmCityType = values.data.ctmCityType;
			...
			if (cityTpeMappingId === "new") {
				masterApi.saveToMaster({ name: 'city-type-mapping', formData: lrData })
			}
			else {
				masterApi.updateToMaster({ name: 'city-type-mapping', id: cityTpeMappingId, formData: lrData })
			}
		}
	...
	function onCancel() {
		//console.log(values);
		props.history.push("/app/master/city-type-mapping/listing");
	}
		...
		<FusePageSimple
						...
						<div className="flex flex-col items-start max-w-full">
							<FuseAnimate animation="transition.slideRightIn" delay={300}>
								<Typography
									...
									component={Link}
									role="button"
									to="/app/master/city-type-mapping/listing"
									color="inherit"
								>
							...
							<div className="flex items-center max-w-full">
								...
								<div className="flex flex-col min-w-0 mx-8 sm:mc-16">
									...
									<FuseAnimate animation="transition.slideLeftIn" delay={300}>
										<Typography variant="caption"><Label labelId="BL00321" /></Typography>
									</FuseAnimate>
								</div>
							</div>
						</div>
			...
			content={
				...
				<div className="p-16 sm:p-24">
					...
					{cityTpeMappingId !== "new" && cityData && cityData.id && (
						<SmartForm
...
export default CityTypeMapping;
