...
function EmployeeCategory(props) {
	...
	const { trngSubcatId } = routeParams;
	const [disable, setDisable] = useState(false);

	useDeepCompareEffect(() => {
		async function updateProductState() {
			console.log("trngSubcatId", trngSubcatId);
			if (trngSubcatId === 'new') {

			} else {
				let apiObj = await masterApi.getTrainingSubCategoryById(trngSubcatId);
				let obj = {
					id: apiObj.trngSubcatId,
					trngSubcatName: apiObj.trngSubcatName,
					trngCatCode: apiObj.trngCatCode,
	...
	function onSubmit(values) {
		...
		if (values.button.toUpperCase() === "SAVE") {
			...
			lrData.trngSubcatName = values.data.trngSubcatName;
			lrData.trngCatCode = values.data.trngCatCode;
			lrData.status = values.data.status.toUpperCase();

			console.log(lrData);

			if (trngSubcatId === "new") {
				masterApi.saveToMaster({ name: 'training-sub-category', formData: lrData })
			}
			else {
				masterApi.updateToMaster({ name: 'training-sub-category', id: trngSubcatId, formData: lrData })
			}
		}
	...
	function onCancel() {
		//console.log(values);
		props.history.push("/app/master/training-sub-category/listing");
	}
						...
						<div className="flex flex-col items-start max-w-full">
							<FuseAnimate animation="transition.slideRightIn" delay={300}>
								<Typography
									...
									component={Link}
									role="button"
									to="/app/master/training-sub-category/listing"
									color="inherit"
								>
							...
							<div className="flex items-center max-w-full">
								...
								<div className="flex flex-col min-w-0 mx-8 sm:mc-16">
									...
									<FuseAnimate animation="transition.slideLeftIn" delay={300}>
										<Typography variant="caption"><Label labelId="BL00368" /></Typography>
									</FuseAnimate>
								</div>
							</div>
						</div>
