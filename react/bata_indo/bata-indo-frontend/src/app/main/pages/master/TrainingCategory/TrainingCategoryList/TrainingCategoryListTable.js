...
import EmployeeCategoryListTableHead from './TrainingCategoryListTableHead';
...
function DepartmentListTable(props) {
	...
	useEffect(() => {
		async function updateProductState() {
			let getApi = await masterApi.getAllTrainingCategory();
			let Array = [];
			_.isArray(getApi)
				&& getApi.map((d) => (Array.push(
					{
						trngCatId: d.trngCatId,
						...d
					}
				)));
			setData(Array);
	...
	function handleClick(item) {
		props.history.push(`/app/master/trainingCategory/create/${item.trngCatId}`);
	}
