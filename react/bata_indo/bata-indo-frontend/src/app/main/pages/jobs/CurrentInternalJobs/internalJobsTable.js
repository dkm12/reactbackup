...
import { getActiveJobsList, selectInternalJobPostings } from '../store/empCurrentVacanciesSlice'
import { Link, useParams } from 'react-router-dom';
import InternalJobsTableHead from './internalJobsTableHead';
...
function InternalJobsTable(props) {
	const dispatch = useDispatch();
	const InternalJobs = useSelector(selectInternalJobPostings);
	// const uuid = useSelector(({ auth }) => auth.user.uuid);
	const totalRecords = useSelector(({ jobs }) => jobs.empCurrentVacancies.totalItems);
	const searchText = useSelector(({ jobs }) => jobs.empCurrentVacancies.searchText);
	const role = useSelector(({ auth }) => auth.user.role);
	const [selected, setSelected] = useState([]);
	const [data, setData] = useState(InternalJobs);
	...
	useEffect(() => {
		if (searchText.length !== 0) {
			setData(_.filter(InternalJobs, item => item.name.toLowerCase().includes(searchText.toLowerCase())));
			setPage(0);
		} else {
			setData(InternalJobs);
		}
	}, [InternalJobs, searchText]);
	...
	function handleClick(id) {
		...
		props.history.push(`/app/jobs/jobposting/applicantsForm/${id}`);
	}
...
export default withRouter(InternalJobsTable);
