...
function Create(props) {
	...
	async function onSubmit(values) {
		...
		if (values.button == "publish") {
			if (jobs && jobs.jbdId) dispatch(saveNewPosting({ type: "publish", data: { ...values.data, jbdId: jobs.jbdId } }));
			else dispatch(saveNewPosting({ type: "publish", data: { ...values.data } }));
		}
		if (jobs && jobs.jbdId && jpId !== null) {
			// console.log(jobs && jobs.id,'job_values');
					...
					<div className="p-16 sm:p-24">
						...
						{jpId == 'new' &&
							<SmartForm
								...
								onCancel={onCancel}
								onChange={values => console.log('outvalues', values)}
								buttons={jobs && jobs.outcome && jobs.outcome.toUpperCase() === "SUBMIT" ? ['cancel'] : ['save', 'cancel']}
							/>
						}
