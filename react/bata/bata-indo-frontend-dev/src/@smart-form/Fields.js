...
function Fields({
	...
	const handelChangeCkEditor = (name, value) => {
		console.log('mmmmmmmmmmmmmmmm', name, value)
	}
		...
		<>
			{fields.map(field => {
				...
				if (defaultValues) {
					...
					// if(type == "autocomplete" && preValue.defaultValue){
					// 	console.log(`preValue `, preValue.defaultValue)
					// 	const res = options.filter(ele=>{return ele.vlaue == preValue.defaultValue})
					// 	preValue.defaultValue = res;
					// }
				}
				...
				// console.log("multiSelect", Object.keys(multiSelect).length);
				// console.log("disabled",Object.keys(disableField).length===0);
				...
				if (type == "autocomplete" && preValue && preValue.defaultValue && !inputAutoValue) {
					...
					// console.log(`preValue.defaultValue 2>>>`, preValue.defaultValue, res[0])
					// setInputAutoValue({...inputAutoValue, [registerName]: res[0]})
				}
				...
				switch (type) {
											...
											<Tooltip title={watch(registerName) && watch(registerName)[0] && watch(registerName)[0].name || "Add File"}>
												<label htmlFor={registerName}>
													<div color={watch(registerName) && watch(registerName)[0] && watch(registerName)[0].name ? "secondry" : "primary"} aria-label="upload picture" size={size} component="span">
														{/* <img src="/app/assets/images/icons/upload.svg" alt="" />  */}
														{/* Choose your file... */}
														{/* <AttachmentIcon /> */}
														<Button
															variant="contained"
					...
					case 'array':
						return (
							<FieldWrapper
								type={layout && layout.type}
								attr={{ xs: xsv, key: name, spacing: spacing, style: { overflowX: 'auto' } }}
								content={
									<>
										<FieldArray
											{...{
