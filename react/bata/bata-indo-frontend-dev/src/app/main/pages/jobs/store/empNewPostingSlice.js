...
export const saveNewPosting = createAsyncThunk('jobs/empJobPosting/saveNewPosting', async (newJobsParams,thunkAPI) => {
	...
	} else if (newJobsParams.type == 'savePublish') {
		console.log("newJobsParams", newJobsParams.data);
		let res = await axios.post(api.jobs.ijpSave, newJobsParams.data);
		if (res.data.status == '200') {
			response = await axios.put(api.jobs.ijpJobPublish + res.data.data.jbdId, { "ijpStatus": "open" });
		}
		else{ response = res }
	}
