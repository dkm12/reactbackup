...
function EmpCornerForm(props) {
				...
				fields: [
					...
					},
					{
						...
						fields: [
							...
							{
								type: 'attachment',
								name: 'topicAttachFileName',
								id: 'topicAttachFileName',
								dynamic: {
									field: 'createdBy',
									value: uuid
								}
	...
	async function onSubmit(values) {
		...
		if (empCorner && empCorner.topicId && topicId !== 'new') {
			dispatch(saveNewEmpCorner({ type: "update", data: { ...values.data, topicId: empCorner.topicId, topicAttachFileName: fileURL } }));
			setLoading(true);
		} else {
			dispatch(saveNewEmpCorner({ type: "save", data: { ...rest, topicAttachFileName: fileURL } }))
			setLoading(true);
		}
	}
	function onCancel() {
