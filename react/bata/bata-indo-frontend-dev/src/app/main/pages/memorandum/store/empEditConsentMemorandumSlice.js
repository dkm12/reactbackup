...
export const getConsentMemorandum = createAsyncThunk('memorandum/empConsentMemorandum/getConsentMemorandum', async params => {
	...
	let consentQuery = "";
	let chAnswer="";
	let consentRaisedBy=-1;
	let objConsent=data.consentList.find(o => o.consentRaisedTo === params.uuid && o.status === "RAISED")
	if(objConsent !== undefined)
	{
		consentRaised=true;
		consentRaisedTo=objConsent.consentRaisedTo;
		consentQuery=objConsent.mchQuery;
		consentRaisedBy=objConsent.consentRaisedBy;
	}
	data.consentRaised=consentRaised;
	data.consentRaisedBy=consentRaisedBy;
	data.mchQuery=consentQuery;

	return data;
});

export const getNextApprover = createAsyncThunk('memorandum/empConsentMemorandum/getNextApprover', async params => {
...
export const saveReplyConsentMemorandum = createAsyncThunk('memorandum/empConsentMemorandum/saveReplyConsentMemorandum', async (claimRequest, thunkAPI) => {
	thunkAPI.dispatch(showSplash({ state: true }));
	const response = await axios.post(api.memorandum.replyConsent, claimRequest);
...
export const sendConsentMemorandum = createAsyncThunk('memorandum/empConsentMemorandum/sendConsentMemorandum', async (claimRequest, thunkAPI) => {
	thunkAPI.dispatch(showSplash({ state: true }));
	...
	if (data.status == "200") {
		thunkAPI.dispatch(showMessage({ message: data.message, variant: 'success' }));
		history.push({
			pathname: '/app/claim-requests/consent-memorandum'
		});
	} else {
		thunkAPI.dispatch(showMessage({ message: data.message, variant: 'error' }));
	}
...
const empConsentMemorandumAdapter = createEntityAdapter({});

const empConsentMemorandumSlice = createSlice({
	name: 'memorandum/empConsentMemorandum',
	initialState: empConsentMemorandumAdapter.getInitialState({
		nextApprovers: []
	}),
	reducers: {
		newConsentMemorandum: {
	...
	extraReducers: {
		[getConsentMemorandum.fulfilled]: (state, action) => action.payload,
		[saveReplyConsentMemorandum.fulfilled]: (state, action) => action.payload,
...
export const { newConsentMemorandum } = empConsentMemorandumSlice.actions;

export default empConsentMemorandumSlice.reducer;
