...
import { getApproveMemorandumListing, selectApproveMemorandumListing } from '../store/empApproveMemorandumListingSlice';
//import ApproveMemorandumTableHead from './ApproveMemorandumTableHead';
...
function ApproveMemorandumTable(props) {
	const dispatch = useDispatch();
	const approveClaims = useSelector(selectApproveMemorandumListing);
	const uuid = useSelector(({ auth }) => auth.user.uuid);
	const searchText = useSelector(({ memorandum }) => memorandum.empApproveMemorandumListing.searchText);
	const totalRecords = useSelector(({ memorandum }) => memorandum.empApproveMemorandumListing.totalRecords);
	...
	useEffect(() => {
		dispatch(getApproveMemorandumListing({
			uuid: uuid,
			page: page,
	...
	function handleClick(item) {
		props.history.push(`/app/claim-requests/approve-memorandum/${item.id}`);
	}
...
export default withRouter(ApproveMemorandumTable);
