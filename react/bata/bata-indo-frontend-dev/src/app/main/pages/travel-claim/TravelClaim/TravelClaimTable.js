...
import { getTravelClaims, selectTravelClaims } from '../store/empTravelClaimsSlice';
//import TravelClaimTableHead from './TravelClaimTableHead';
...
function TravelClaimTable(props) {
	const dispatch = useDispatch();
	const travelClaims = useSelector(selectTravelClaims);
	const uuid = useSelector(({ auth }) => auth.user.uuid);
	const searchText = useSelector(({ travelClaim }) => travelClaim.empTravelClaims.searchText);
	const totalRecords = useSelector(({ travelClaim }) => travelClaim.empTravelClaims.totalRecords);
	...
	function handleClick(item) {
		props.history.push(`/app/employee-service/travel-claim/${item.id}`);
	}
...
export default withRouter(TravelClaimTable);
