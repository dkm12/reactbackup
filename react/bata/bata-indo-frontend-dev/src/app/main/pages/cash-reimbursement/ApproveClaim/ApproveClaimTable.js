...
function ApproveClaimTable(props) {
	...
	const searchText = useSelector(({ cashReimbursement }) => cashReimbursement.empApproveClaims.searchText);
	const totalRecords = useSelector(({ cashReimbursement }) => cashReimbursement.empApproveClaims.totalRecords);
	...
	if (data) {
		data.map(n => {
			csvdata.push({
				...
				claimedTotalAmt: n.totalAmt,
				statusCode: n.statusName,
			})
		})
	}
	...
	function handleClick(item) {
		props.history.push(`/app/claim-requests/approve-cash-reimbursement/${item.id}`);
	}
	...
	return (
		<div className="w-full flex flex-col">
			...
			<CSVDownloadComp data={csvdata} header={csvheaders} filename='cash_reimbursement'/>
