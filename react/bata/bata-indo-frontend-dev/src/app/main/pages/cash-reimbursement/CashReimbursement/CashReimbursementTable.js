...
import { getCashReimbursements, selectCashReimbursements } from '../store/empCashReimbursementsSlice';
import CashReimbursementTableHead from './CashReimbursementTableHead';
...
function CashReimbursementTable(props) {
	const dispatch = useDispatch();
	const cashReimbursements = useSelector(selectCashReimbursements);
	const uuid = useSelector(({ auth }) => auth.user.uuid);
	const searchText = useSelector(({ cashReimbursement }) => cashReimbursement.empCashReimbursements.searchText);
	const totalRecords = useSelector(({ cashReimbursement }) => cashReimbursement.empCashReimbursements.totalRecords);
	...
	useEffect(() => {
		dispatch(getCashReimbursements({
			uuid: uuid,
			page: page,
	...
	function handleClick(item) {
		props.history.push(`/app/employee-service/cash-reimbursement/${item.id}`);
	}
...
export default withRouter(CashReimbursementTable);
