...
function TravelEntitlement(props) {
	...
	useDeepCompareEffect(() => {
		async function updateProductState() {
			...
			if (teId === 'new') {
				...
				let apiObj = await masterApi.getTravelEntitlementById(teId);
				let obj = {
					id: apiObj.teId,
					teEmpCatCode: apiObj.teEmpCatCode,
	...
	let template = {
		...
		sections: [
			{
				id: 'travelEntitlement',
				fields: [{
					type: 'text',
					id: 'teEmpCatCode',
					name: 'teEmpCatCode',
					title: 'Travel Entitlement Name'
				}, {
					type: 'radio',
					id: 'status',
					name: 'status',
	...
	function onSubmit(values) {
		...
		if (values.button.toUpperCase() === "SAVE") {
			...
			lrData.teEmpCatCode = values.data.teEmpCatCode;
			lrData.status = values.data.status.toUpperCase();

			console.log(lrData);

			if (teId === "new") {
				masterApi.saveToMaster({ name: 'travel-entitlement', formData: lrData })
			}
			else {
				masterApi.updateToMaster({ name: 'travel-entitlement', id: teId, formData: lrData })
			}
		}
	}
	...
	function onCancel() {
		//console.log(values);
		props.history.push("/app/master/travel-entitlement/listing");
	}
						...
						<div className="flex flex-col items-start max-w-full">
							<FuseAnimate animation="transition.slideRightIn" delay={300}>
								<Typography
									...
									component={Link}
									role="button"
									to="/app/master/travel-entitlement/listing"
									color="inherit"
								>
									<Icon className="text-20">
										{theme.direction === 'ltr' ? 'arrow_back' : 'arrow_forward'}
									</Icon>
									<span className="mx-4">Masters</span>
								</Typography>
							</FuseAnimate>
							...
							<div className="flex items-center max-w-full">
								...
								<div className="flex flex-col min-w-0 mx-8 sm:mc-16">
									...
									<FuseAnimate animation="transition.slideLeftIn" delay={300}>
										<Typography variant="caption">Travel Entitlement</Typography>
									</FuseAnimate>
								</div>
							</div>
						</div>
