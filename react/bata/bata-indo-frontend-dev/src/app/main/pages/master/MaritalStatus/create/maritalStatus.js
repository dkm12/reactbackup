...
function MaritalStatus(props) {
	...
	useDeepCompareEffect(() => {
		async function updateProductState() {
			...
			if (msId === 'new') {
				...
				let apiObj = await masterApi.getmaritalStatusById(msId);
				let obj = {
					id: apiObj.msId,
					msName: apiObj.msName,
	...
	function onSubmit(values) {
		...
		if (values.button.toUpperCase() === "SAVE") {
			...
			lrData.msName = values.data.msName;
			lrData.status = values.data.status.toUpperCase();

			console.log(lrData);
			if (msId === "new") {
				masterApi.saveToMaster({ name: 'marital-status', formData: lrData })
			}
			else {
				masterApi.updateToMaster({ name: 'marital-status', id: msId, formData: lrData })
			}
	...
	function onCancel() {
		//console.log(values);
		props.history.push("/app/master/marital-status/listing");
	}
						...
						<div className="flex flex-col items-start max-w-full">
							<FuseAnimate animation="transition.slideRightIn" delay={300}>
								<Typography
									...
									component={Link}
									role="button"
									to="/app/master/marital-status/listing"
									color="inherit"
								>
							...
							<div className="flex items-center max-w-full">
								...
								<div className="flex flex-col min-w-0 mx-8 sm:mc-16">
									...
									<FuseAnimate animation="transition.slideLeftIn" delay={300}>
										<Typography variant="caption"><Label labelId="BL00282" /></Typography>
									</FuseAnimate>
								</div>
							</div>
						</div>
