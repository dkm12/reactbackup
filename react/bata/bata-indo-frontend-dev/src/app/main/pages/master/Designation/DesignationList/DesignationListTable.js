...
import DesignationListTableHead from './DesignationListTableHead';
...
function DesignationListTable(props) {
	const [data, setData] = useState([]);
	useEffect(() => {
		async function updateProductState() {
			let getAlldesignations = await masterApi.getAlldesignation();
			let Array = [];
			_.isArray(getAlldesignations)
				&& getAlldesignations.map((d) => (Array.push(
					{
						dsgId: d.dsgId,
						...d
					}
	...
	function handleClick(item) {
		props.history.push(`/app/master/designation/create/${item.dsgId}`);
	}
...
export default withRouter(DesignationListTable);
