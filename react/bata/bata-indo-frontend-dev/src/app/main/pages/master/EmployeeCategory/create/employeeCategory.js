...
function EmployeeCategory(props) {
	...
	useDeepCompareEffect(() => {
		async function updateProductState() {
			...
			if (ecId === 'new') {
				...
				let apiObj = await masterApi.getEmployeeCategoryById(ecId);
				let obj = {
					id: apiObj.ecId,
					ecName: apiObj.ecName,
	...
	function onSubmit(values) {
		...
		if (values.button.toUpperCase() === "SAVE") {
			...
			lrData.ecName = values.data.ecName;
			lrData.status = values.data.status.toUpperCase();

			console.log(lrData);

			if (ecId === "new") {
				masterApi.saveToMaster({ name: 'employee-category', formData: lrData })
			}
			else {
				masterApi.updateToMaster({ name: 'employee-category', id: ecId, formData: lrData })
			}
		}
	...
	function onCancel() {
		//console.log(values);
		props.history.push("/app/master/employee-category/listing");
	}
						...
						<div className="flex flex-col items-start max-w-full">
							<FuseAnimate animation="transition.slideRightIn" delay={300}>
								<Typography
									...
									component={Link}
									role="button"
									to="/app/master/employee-category/listing"
									color="inherit"
								>
							...
							<div className="flex items-center max-w-full">
								...
								<div className="flex flex-col min-w-0 mx-8 sm:mc-16">
									...
									<FuseAnimate animation="transition.slideLeftIn" delay={300}>
										<Typography variant="caption"><Label labelId="BL00265" /></Typography>
									</FuseAnimate>
								</div>
							</div>
						</div>
