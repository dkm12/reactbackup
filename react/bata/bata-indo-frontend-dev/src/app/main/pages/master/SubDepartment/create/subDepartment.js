...
function SubDepartment(props) {
	...
	useDeepCompareEffect(() => {
		async function updateProductState() {
			...
			_.isArray(DepartmentData)
				&& DepartmentData.map((d) => (dpt.push({ title: d.dptCode })));
			setDepartment(dpt);
			console.log("sbdId", sbdId);
			if (sbdId === 'new') {

			} else {
				let apiObj = await masterApi.getSubDepartmentById(sbdId);
				let obj = {
					id: apiObj.sbdId,
					sbdName: apiObj.sbdName,
					dptCode: apiObj.dptCode,
	...
	function onSubmit(values) {
		...
		if (values.button.toUpperCase() === "SAVE") {
			...
			lrData.sbdName = values.data.sbdName;
			lrData.dptCode = values.data.dptCode;
			lrData.status = values.data.status.toUpperCase();

			console.log(lrData);

			if (sbdId === "new") {
				masterApi.saveToMaster({ name: 'subDepartment', formData: lrData })
			}
			else {
				masterApi.updateToMaster({ name: 'subDepartment', id: sbdId, formData: lrData })
			}
	...
	function onCancel() {
		//console.log(values);
		props.history.push("/app/master/subDepartment/listing");
	}
						...
						<div className="flex flex-col items-start max-w-full">
							<FuseAnimate animation="transition.slideRightIn" delay={300}>
								<Typography
									...
									component={Link}
									role="button"
									to="/app/master/subDepartment/listing"
									color="inherit"
								>
							...
							<div className="flex items-center max-w-full">
								...
								<div className="flex flex-col min-w-0 mx-8 sm:mc-16">
									...
									<FuseAnimate animation="transition.slideLeftIn" delay={300}>
										<Typography variant="caption"><Label labelId="BL00269" /></Typography>
									</FuseAnimate>
								</div>
							</div>
						</div>
