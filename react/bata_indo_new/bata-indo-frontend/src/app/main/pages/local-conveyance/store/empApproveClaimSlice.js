...
export const getApproveClaim = createAsyncThunk('localConveyance/empApproveClaim/getApproveClaim', async params => {
    ...
    //     }
    //     else {
    //         const responseLcClaimHr = await axios.get(api.localConveyance.getClaimHr);
    //         let nextApproverArray = await responseLcClaimHr.data.data;
    //         const nextApprover = [];
    //         _.isArray(nextApproverArray)
    //             && nextApproverArray.map((d) => (nextApprover.push({ value: d.employId, title: d.fullname + " (" + d.employId + ")" })));
    //         data.nextApprover = nextApprover;
    ...
    // }
    // if (data.statusCode === "PENDING_WITH_CLAIM_HR") {
    //     const responseLcFinanceOne = await axios.get(api.localConveyance.getLocalConveyanceFinanceOne);
    //     let nextApproverArray = await responseLcFinanceOne.data.data;
    //     const nextApprover = [];
    //     _.isArray(nextApproverArray)
    ...
    //     data.nextApprover = nextApprover;
    // }
    if ((data.statusCode === "PENDING_WITH_HOD" && data.totalAmt < 1000000) || (data.statusCode === "PENDING_WITH_CLAIM_HR")) {
        const responseLcFinanceOne = await axios.get(api.localConveyance.getLocalConveyanceFinanceOne);
        let nextApproverArray = await responseLcFinanceOne.data.data;
        const nextApprover = [];
        _.isArray(nextApproverArray)
    ...
    if (data.statusCode === "PENDING_WITH_HOD" && data.totalAmt >= 1000000) {
        const responseLcFinanceOne = await axios.get(api.localConveyance.getClaimHr);
        let nextApproverArray = await responseLcFinanceOne.data.data;
        const nextApprover = [];
        _.isArray(nextApproverArray)
            && nextApproverArray.map((d) => (nextApprover.push({ value: d.employId, title: d.fullname + " (" + d.employId + ")" })));
    ...
    if (data.statusCode === "PENDING_WITH_FINANCE_DIR" && data.totalAmt >= 10000000) {
