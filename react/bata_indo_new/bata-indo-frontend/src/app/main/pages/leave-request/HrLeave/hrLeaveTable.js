...
import { getApproveLeavesHR, selectApproveLeaves } from '../store/hrLeaveListSlice';
...
import SearchFilter from '../ApproveLeave/filter';
...
function ApproveLeaveTable(props) {
	...
	const totalRecords = useSelector(({ approveLeave }) => approveLeave.empApproveLeavesHR.totalRecords);
	...
	if (data) {
		data.map(n => {
			csvdata.push({
				...
				leaveFrom: dateFunc.changeDate(n.leaveFrom),
				leaveTo: dateFunc.changeDate(n.leaveTo),
				statusName: n.statusName,
			})
		})
	}
if(csvdata) console.log(csvdata)
	useEffect(() => {
		dispatch(getApproveLeavesHR({
			role: userRole,
			uuid: uuid,
			page: page,
	...
	return (
		<div className="w-full flex flex-col">
			<FuseScrollbars className="flex-grow overflow-x-auto">
				...
				<CSVDownloadComp data={csvdata} header={csvheaders} filename='Approve_Empoloyee_Leave' />
