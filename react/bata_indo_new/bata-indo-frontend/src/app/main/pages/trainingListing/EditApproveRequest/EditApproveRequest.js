...
import { saveApproveRequest, getApproveRequest, getNextApprover } from '../store/empApproveRequestSlice';
...
function EditApproveApproveRequest(props) {
	const dispatch = useDispatch();
	const uuid = useSelector(({ auth }) => auth.user.uuid);
	const userName = useSelector(({ auth }) => auth.user.name);
	const training = useSelector(({ training }) => training.empApproveRequest);
	...
	useDeepCompareEffect(() => {
		async function updateProductState() {
			...
			dispatch(getApproveRequest(params));
	...
	if (training && 'nextApprover' in training
		&& training.statusCode.toUpperCase() === "PENDING_WITH_TRNG_APPROVER"
	...
	function onSubmit(values) {
		...
		if (values.button.toUpperCase() === "ACCEPT") {
			...
			if (values.data.statusCode.toUpperCase() == "PENDING_WITH_RM".toUpperCase()) {
			...
			dispatch(saveApproveRequest(lrData));
			setLoading(true);
	...
	function onCancel() {
		//console.log(values);
		props.history.push("/app/claim-requests/training-requests");
	}
					...
					(
						<div className="flex flex-1 w-full items-center justify-between">
							<div className="flex flex-col items-start max-w-full">
								<FuseAnimate animation="transition.slideRightIn" delay={300}>
									<Typography
										...
										component={Link}
										role="button"
										to="/app/claim-requests/training-requests"
										color="inherit"
									>
										<Icon className="text-20">
											{theme.direction === 'ltr' ? 'arrow_back' : 'arrow_forward'}
										</Icon>
										<span className="mx-4"><Label labelId="BL00376" /></span>
									</Typography>
								</FuseAnimate>
									...
									<div className="flex flex-col min-w-0 mx-8 sm:mc-16">
										<FuseAnimate animation="transition.slideLeftIn" delay={300}>
											<Typography className="text-16 sm:text-20 truncate">
												<Label labelId="BL00376" />
											</Typography>
										</FuseAnimate>
										{/* <FuseAnimate animation="transition.slideLeftIn" delay={300}>
											<Typography variant="caption"><Label labelId="BL00376" />t</Typography>
										</FuseAnimate> */}
									</div>
								</div>
							</div>
							...
							<Button
								...
								onClick={() => dispatch(saveApproveRequest(form))}
							>
								Save
							</Button>
						</FuseAnimate> */}
...
export default withReducer('training', reducer)(EditApproveApproveRequest);
