...
import { getTrainingApplicationListing, selectTrainingApplicationListing } from '../store/empTrainingApplicationListingSlice';
//import TrainingApplicationListingTableHead from './TrainingApplicationListingTableHead';
...
const columns = [
	...
	function TrainingApplicationListingTable(props) {
	const dispatch = useDispatch();
	const approveClaims = useSelector(selectTrainingApplicationListing);
	const uuid = useSelector(({ auth }) => auth.user.uuid);
	const searchText = useSelector(({ training }) => training.empTrainingApplicationListing.searchText);
	const totalRecords = useSelector(({ training }) => training.empTrainingApplicationListing.totalRecords);
	...
	useEffect(() => {
		dispatch(getTrainingApplicationListing({
			uuid: uuid,
			page: page,
	...
	function handleClick(item) {
		props.history.push(`/app/hr-services/training-applications/${item.id}`);
	}
							...
							.map(n => {
								...
								return (
									...
									>
										...
										<TableCell className="p-4 md:p-16" component="th" scope="row" align="left">
											{n.statusCode.toUpperCase() == "PENDING_WITH_TRNG_ADMIN".toUpperCase()
											|| n.statusCode.toUpperCase() == "ENROLLED".toUpperCase()
												?
...
export default withRouter(TrainingApplicationListingTable);
