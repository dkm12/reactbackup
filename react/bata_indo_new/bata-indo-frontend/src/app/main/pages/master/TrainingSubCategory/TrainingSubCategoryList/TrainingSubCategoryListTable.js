...
import EmployeeCategoryListTableHead from './TrainingSubCategoryListTableHead';
...
function DepartmentListTable(props) {
	...
	useEffect(() => {
		async function updateProductState() {
			let getApi = await masterApi.getAllTrainingSubCategory();
			let Array = [];
			_.isArray(getApi)
				&& getApi.map((d) => (Array.push(
					{
						trngSubcatId: d.trngSubcatId,
						...d
					}
				)));
			setData(Array);
	...
	function handleClick(item) {
		props.history.push(`/app/master/trainingSubCategory/create/${item.trngSubcatId}`);
	}
