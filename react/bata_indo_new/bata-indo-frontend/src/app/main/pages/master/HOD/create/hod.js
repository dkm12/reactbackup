...
function Hod(props) {
	...
	useDeepCompareEffect(() => {
		async function updateProductState() {
			...
			if (hodId === 'new') {
				...
				let apiObj = await masterApi.getHodById(hodId);
				let obj = {
					id: apiObj.hodId,
					hodName: apiObj.hodName,
	...
	function onSubmit(values) {
		...
		if (values.button.toUpperCase() === "SAVE") {
			...
			lrData.hodName = values.data.hodName;
			lrData.status = values.data.status.toUpperCase();

			console.log(lrData);
			if (hodId === "new") {
				masterApi.saveToMaster({ name: 'hod', formData: lrData })
			}
			else {
				masterApi.updateToMaster({ name: 'hod', id: hodId, formData: lrData })
			}
		}
	}
	...
	function onCancel() {
		//console.log(values);
		props.history.push("/app/master/hod/listing");
	}
						...
						<div className="flex flex-col items-start max-w-full">
							...
							<div className="flex items-center max-w-full">
								...
								<div className="flex flex-col min-w-0 mx-8 sm:mc-16">
									...
									<FuseAnimate animation="transition.slideLeftIn" delay={300}>
										<Typography variant="caption"><Label labelId="BL00274" /></Typography>
									</FuseAnimate>
								</div>
							</div>
						</div>
