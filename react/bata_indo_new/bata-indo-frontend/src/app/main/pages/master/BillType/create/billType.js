...
function BillType(props) {
	...
	const { billTypeId } = routeParams;
	const [disable, setDisable] = useState(false);


	useDeepCompareEffect(() => {
		async function updateProductState() {
			console.log("billTypeId", billTypeId);
			if (billTypeId === 'new') {

			} else {
				let billObj = await masterApi.getBillById(billTypeId);
				let obj = {
					id: billObj.btId,
					btName: billObj.btName,
					status: billObj.status.toLowerCase()
				};
				console.log("obj", obj);
				setBillData(obj);
			}
	...
	function onSubmit(values) {
		...
		if (values.button.toUpperCase() === "SAVE") {
			...
			lrData.btName = values.data.btName;
	...
	function onCancel() {
		//console.log(values);
		props.history.push("/app/master/bill-type/listing");
	}
		...
		<FusePageSimple
						...
						<div className="flex flex-col items-start max-w-full">
							<FuseAnimate animation="transition.slideRightIn" delay={300}>
								<Typography
									...
									component={Link}
									role="button"
									to="/app/master/bill-type/listing"
									color="inherit"
								>
							...
							<div className="flex items-center max-w-full">
								...
								<div className="flex flex-col min-w-0 mx-8 sm:mc-16">
									...
									<FuseAnimate animation="transition.slideLeftIn" delay={300}>
										<Typography variant="caption"><Label labelId="BL00318" /></Typography>
									</FuseAnimate>
								</div>
							</div>
						</div>
			...
			content={
				<div className="p-16 sm:p-24">
					...
					{billTypeId !== "new" && billData && billData.id && (
						<SmartForm
