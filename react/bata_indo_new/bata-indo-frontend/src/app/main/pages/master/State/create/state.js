...
function State(props) {
	...
	useDeepCompareEffect(() => {
		async function updateProductState() {
			...
			if (stateId === 'new') {
				...
				let apiObj = await masterApi.getStateById(stateId);
				let obj = {
					id: apiObj.stateId,
					stateName: apiObj.stateName,
	...
	function onSubmit(values) {
		...
		if (values.button.toUpperCase() === "SAVE") {
			...
			lrData.stateName = values.data.stateName;
			lrData.status = values.data.status.toUpperCase();

			console.log(lrData);
			if (stateId === "new") {
				masterApi.saveToMaster({ name: 'state', formData: lrData })
			}
			else {
				masterApi.updateToMaster({ name: 'state', id: stateId, formData: lrData })
			}
	...
	function onCancel() {
		//console.log(values);
		props.history.push("/app/master/state/listing");
	}
						...
						<div className="flex flex-col items-start max-w-full">
							...
							<div className="flex items-center max-w-full">
								...
								<div className="flex flex-col min-w-0 mx-8 sm:mc-16">
									...
									<FuseAnimate animation="transition.slideLeftIn" delay={300}>
										<Typography variant="caption"><Label labelId="BL00271" /></Typography>
									</FuseAnimate>
								</div>
							</div>
						</div>
