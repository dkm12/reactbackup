...
function Zone(props) {
	...
	useDeepCompareEffect(() => {
		async function updateProductState() {
			...
			if (zoneId === 'new') {
				...
				let apiObj = await masterApi.getzoneById(zoneId);
				let obj = {
					id: apiObj.zoneId,
					zoneName: apiObj.zoneName,
	...
	function onSubmit(values) {
		...
		if (values.button.toUpperCase() === "SAVE") {
			...
			lrData.zoneName = values.data.zoneName;
			lrData.status = values.data.status.toUpperCase();

			console.log(lrData);
			if (zoneId === "new") {
				masterApi.saveToMaster({ name: 'zone', formData: lrData })
			}
			else {
				masterApi.updateToMaster({ name: 'zone', id: zoneId, formData: lrData })
			}
	...
	function onCancel() {
		//console.log(values);
		props.history.push("/app/master/zone/listing");
	}
						...
						<div className="flex flex-col items-start max-w-full">
							...
							<div className="flex items-center max-w-full">
								...
								<div className="flex flex-col min-w-0 mx-8 sm:mc-16">
									...
									<FuseAnimate animation="transition.slideLeftIn" delay={300}>
										<Typography variant="caption"><Label labelId="BL00361" /></Typography>
									</FuseAnimate>
								</div>
							</div>
						</div>
