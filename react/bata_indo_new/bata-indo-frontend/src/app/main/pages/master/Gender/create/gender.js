...
function Gender(props) {
	...
	useDeepCompareEffect(() => {
		async function updateProductState() {
			...
			if (genderId === 'new') {
				...
				let apiObj = await masterApi.getGenderById(genderId);
				let obj = {
					id: apiObj.genderId,
					genderName: apiObj.genderName,
	...
	function onSubmit(values) {
		...
		if (values.button.toUpperCase() === "SAVE") {
			...
			lrData.genderName = values.data.genderName;
			lrData.status = values.data.status.toUpperCase();

			console.log(lrData);
			if (genderId === "new") {
				masterApi.saveToMaster({ name: 'gender', formData: lrData })
			}
			else {
				masterApi.updateToMaster({ name: 'gender', id: genderId, formData: lrData })
			}
		}
	...
	function onCancel() {
		//console.log(values);
		props.history.push("/app/master/gender/listing");
	}
						...
						<div className="flex flex-col items-start max-w-full">
							...
							<div className="flex items-center max-w-full">
								...
								<div className="flex flex-col min-w-0 mx-8 sm:mc-16">
									...
									<FuseAnimate animation="transition.slideLeftIn" delay={300}>
										<Typography variant="caption"><Label labelId="BL00334" /></Typography>
									</FuseAnimate>
								</div>
							</div>
						</div>
