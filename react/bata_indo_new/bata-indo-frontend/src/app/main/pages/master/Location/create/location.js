...
function Location(props) {
	...
	useDeepCompareEffect(() => {
		async function updateProductState() {
			...
			if (locId === 'new') {
				...
				let apiObj = await masterApi.getlocationById(locId);
				let obj = {
					id: apiObj.locId,
					locName: apiObj.locName,
	...
	function onSubmit(values) {
		...
		if (values.button.toUpperCase() === "SAVE") {
			...
			lrData.locName = values.data.locName;
			lrData.status = values.data.status.toUpperCase();

			console.log(lrData);
			if (locId === "new") {
				masterApi.saveToMaster({ name: 'location', formData: lrData })
			}
			else {
				masterApi.updateToMaster({ name: 'location', id: locId, formData: lrData })
			}
	...
	function onCancel() {
		//console.log(values);
		props.history.push("/app/master/location/listing");
	}
						...
						<div className="flex flex-col items-start max-w-full">
							<FuseAnimate animation="transition.slideRightIn" delay={300}>
								<Typography
									...
									component={Link}
									role="button"
									to="/app/master/location/listing"
									color="inherit"
								>
