...
function Designation(props) {
	...
	const { dsgId } = routeParams;
	const [disable, setDisable] = useState(false);

	useDeepCompareEffect(() => {
		async function updateProductState() {
			console.log("dsgId", dsgId);
			if (dsgId === 'new') {

			} else {
				let apiObj = await masterApi.getdesignationById(dsgId);
				let obj = {
					id: apiObj.dsgId,
					dsgName: apiObj.dsgName,
	...
	function onSubmit(values) {
		...
		if (values.button.toUpperCase() === "SAVE") {
			...
			lrData.dsgName = values.data.dsgName;
			lrData.status = values.data.status.toUpperCase();

			console.log(lrData);

			if (dsgId === "new") {
				masterApi.saveToMaster({ name: 'designation', formData: lrData })
			}
			else {
				masterApi.updateToMaster({ name: 'designation', id: dsgId, formData: lrData })
			}
		}
	...
	function onCancel() {
		//console.log(values);
		props.history.push("/app/master/designation/listing");
	}
						...
						<div className="flex flex-col items-start max-w-full">
							<FuseAnimate animation="transition.slideRightIn" delay={300}>
								<Typography
									...
									component={Link}
									role="button"
									to="/app/master/designation/listing"
									color="inherit"
								>
							...
							<div className="flex items-center max-w-full">
								...
								<div className="flex flex-col min-w-0 mx-8 sm:mc-16">
									...
									<FuseAnimate animation="transition.slideLeftIn" delay={300}>
										<Typography variant="caption"><Label labelId="BL00166" /></Typography>
									</FuseAnimate>
								</div>
							</div>
						</div>
