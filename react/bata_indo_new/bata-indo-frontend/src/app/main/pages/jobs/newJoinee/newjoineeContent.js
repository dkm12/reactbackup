...
function JoineeFormContent(props) {
	...
	async function onBlur(val){
		...
		if(val.empCode) {
			let postData = {}
			postData.type = "EMP_CODE";
			postData.value = val.empCode
			let res = await referralApi.checkDuplicate(postData);
			(res.status == '404') ? empCodeValid = true : empCodeValid = false
		}
		if(val.officialEmailId) {
			let postData = {}
			postData.type = "EMAIL_ID";
			postData.value = val.officialEmailId
			let res = await referralApi.checkDuplicate(postData);
			(res.status == '404') ? emailValid = true : emailValid = false; 
		}
		if(val.username) {
			let postData = {}
			postData.type = "USER_NAME";
			postData.value = val.username
			let res = await referralApi.checkDuplicate(postData);
			(res.status == '404') ? userNameValid = true : userNameValid = false;
		}
		if(userNameValid && emailValid && empCodeValid){
			setCheckValid(true)
		}
		else{
			setCheckValid(false)
			let msg = ''
			if(!empCodeValid){
				(msg != '') ? msg = msg+', Employee Code' : msg = 'Employee Code'
			} if(!emailValid){
				(msg != '') ? msg = msg+', Email Id' : msg = 'Email Id'
			} if(!userNameValid){
				(msg != '') ? msg = msg+', User Name' : msg = 'User Name'
			} 
			setValidationAlert('This '+msg+" already exists or not valid")
			dispatch(showMessage({ message: 'This '+msg+" already exists or not valid", variant: 'error' }));
		}
	}
	async function onSubmit(values) {
		if(!checkValid){
			setSuccess(true)
			dispatch(showMessage({ message: validationAlert, variant: 'error' }));
			return;
		}
		...
		let res = await referralApi.saveNewJoineeWithoutRefData(postData);
		if (res.status == '200') {
