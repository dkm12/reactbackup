...
function ApplicantsListTable(props) {
	...
	if(columns){
		columns.map(n => {
			if(n.sort == true) csvheaders.push({title: GetLabel([n.label.props.labelId][0]), field: n.id})
		})
	}
	if (data) {
		data.map(n => {
			csvdata.push({
				...
				fullname: n.fullname,
				ijpCreatedOn: dateFunc.changeDate(n.ijpCreatedOn),
				currentStatusName: n.currentStatusName
			})
		})
	}	
	...
	return (
		<div className="w-full flex flex-col">
			...
			<CSVDownloadComp data={csvdata} header={csvheaders} filename='internal_applicants'/>
							...
							.map(n => {
								...
								return (
									...
									>
										...
										<TableCell className="p-4 md:p-16" component="th" scope="row" align="left">
											<a className={classes.linkBtn} onClick={e=>download(n.ijpResumeFileName)}>
