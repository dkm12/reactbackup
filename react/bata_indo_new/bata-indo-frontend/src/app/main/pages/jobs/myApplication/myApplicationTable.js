...
import MyAppTableHead from './myApplicationTableHead';
...
function MyAppTable(props) {
	const classes = useStyles();
	const dispatch = useDispatch();
	const IJPJobPosting = useSelector(selectMyIjpApp);
	const totalRecords = useSelector(({ applicants }) => applicants.myIjpApplication.totalRecords);
	const [selected, setSelected] = useState([]);
	const [data, setData] = useState([]);
	const [page, setPage] = useState(0);
	const [rowsPerPage, setRowsPerPage] = useState(10);
	const [jobType, setJobType] = useState('IJP');
							...
							.map(n => {
								...
								return (
									...
									>
										...
										<TableCell className="p-4 md:p-16" component="th" scope="row" align="left">
											{dateFunc.changeDate(n.ijpCreatedOn)}
										</TableCell>
