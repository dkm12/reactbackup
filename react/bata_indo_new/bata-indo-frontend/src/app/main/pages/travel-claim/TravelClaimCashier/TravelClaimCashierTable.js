...
import { getTravelClaimCashiers, selectTravelClaimCashiers } from '../store/empTravelClaimCashiersSlice';
import TravelClaimCashierTableHead from './TravelClaimCashierTableHead';
...
function TravelClaimCashierTable(props) {
	...
	const travelClaims = useSelector(selectTravelClaimCashiers);
	const uuid = useSelector(({ auth }) => auth.user.uuid);
	const searchText = useSelector(({ travelClaim }) => travelClaim.empTravelClaimCashiers.searchText);
	const totalRecords = useSelector(({ travelClaim }) => travelClaim.empTravelClaimCashiers.totalRecords);

	
	const [selected, setSelected] = useState([]);
	const [data, setData] = useState(travelClaims);
	...
	if (data) {
		data.map(n => {
			csvdata.push({
				...
				claimedTotalAmt: n.claimedTotalAmt,
	...
	useEffect(() => {
		dispatch(getTravelClaimCashiers(
			{
				uuid: uuid,
				page: page,
	...
	useEffect(() => {
		if (searchText.length !== 0) {
			// setData(_.filter(travelClaims, item => item.name.toLowerCase().includes(searchText.toLowerCase())));
			// setPage(0);
		} else {
			setData(travelClaims);
		}
	}, [travelClaims, searchText]);
	...
	function handleClick(item) {
		props.history.push(`/app/cashier-requests/travel-claim/${item.id}`);
	}
	...
	return (
		<div className="w-full flex flex-col">
			<FuseScrollbars className="flex-grow overflow-x-auto">
				...
				<CSVDownloadComp data={csvdata} header={csvheaders} filename='Domestic_travel_claim' />
