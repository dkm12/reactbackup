...
function MydialogForm({ data }) {
    ...
    async function getGraph() {
        ...
        if (resp.status == '200' && resp.data) {
            ...
            await resp.data.map((graph, i) => {
                (graph.pollpercentage) ? abc.push(graph.pollpercentage.toFixed(0)) : abc.push(0)
            })
            // let temp = ['Option: A','Option: B','Option: C','Option: D'];
            await data.surveyOptions.map((opt, i) => {
                if(opt.surveyOptDesc && opt.surveyOptDesc.trim()) {
                    temp.push('Option ' + (i + 10).toString(36).toUpperCase());
                    (abc[i]) ? temp_percent.push(Number(abc[i]).toFixed(0)) : temp_percent.push(0)
                }
            })
            setpieData({
                datasets:
    ...
    function radioCheck(surveyId, optionId, applied) {
    ...
    return (
        <Grid container spacing={16}>
            <Grid xs={12} sm={6}>
                ...
                {data.surveyOptions &&
                    data.surveyOptions.map((opt, i) => {
                        return (
                            ...
                            <div className={data.applied ? classes.faded : ''}>
                            <div className={((opt.optionId == optionID) ? classes.queRadio2 : classes.queRadio)}
                                onClick={e => radioCheck(opt.surveyId, opt.optionId, data.applied)}>
                ...
                <Button className="w-full" component="button"
                    disabled={data.applied}
                    variant="contained" size="small" color="primary" onClick={() => {
                        handleSaveAns();
                    }}>{(data.applied) ? 'Submitted' : 'Submit'}</Button>
            </Grid>
