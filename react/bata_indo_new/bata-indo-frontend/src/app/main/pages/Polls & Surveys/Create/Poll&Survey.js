...
function CreatePoll(props) {
    ...
    async function onUpdate(val) {
        ...
        let temp = {}
        if (type == 'editable') {
            temp.surveyId = formData.surveyId
            temp.title = val.data.title
            temp.description = val.data.description
            temp.question = val.data.question
            temp.locations = (_.isArray(val.data.locations) ? val.data.locations.join(",") : val.data.locations)
            temp.genders = (_.isArray(val.data.genders) ? val.data.genders.join(",") : val.data.genders)
            temp.departments = (_.isArray(val.data.departments) ? val.data.departments.join(",") : val.data.departments)
            temp.categorys = (_.isArray(val.data.categorys) ? val.data.categorys.join(",") : val.data.categorys)
            temp.surveyOptions = tempOption
        }
        else { temp = formData }

        temp.status = (val.button.toUpperCase() === "PUBLISH") ? 'Published' : formData.status
        temp.publishedFromDate = val.data.publishedFromDate
        temp.publishedToDate = val.data.publishedToDate

        let res = await pollSurveyApi.updateById(formData.surveyId, temp);
        if (res.status == '200') {
            if (val.button.toUpperCase() === "PUBLISH") {
                let postData = {}
                postData.surveyStatus = "Published"
                res = await pollSurveyApi.updateStatusById(formData.surveyId, postData);
            }
        }
        if (res.status == '200') {
    ...
    async function onSubmit(values) {
        if (values.button.toUpperCase() == "SAVE" || values.button.toUpperCase() == "PUBLISH") {
            let tempData = {};
            ...
            if (res.status == '200') {
                if (values.button.toUpperCase() === "PUBLISH") {
                    let postData = {}
                    postData.surveyStatus = "Published"
                    res = await pollSurveyApi.updateStatusById(res.data.surveyId, postData);
                }
            }
            if (res.status == '200') {
                await dispatch(showMessage({ message: res.message, variant: 'success' }));
