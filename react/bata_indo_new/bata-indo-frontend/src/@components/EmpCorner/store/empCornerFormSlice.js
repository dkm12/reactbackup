...
export const topicdisLikes = createAsyncThunk('emp-corner/empCornerForm/saveTopicdisLike', async (topicId, thunkAPI) => {
    const response = await axios.post(api.empCorner.topicdisLikes, { "topicId": topicId });
    const data = await response.data;
    if (data.status == "200") {
        thunkAPI.dispatch(showMessage({ message: data.message, variant: 'success' }));
        const response = await axios.get(api.empCorner.getByIdTopic + '/' + topicId);
...
export const CommentdisLikes = createAsyncThunk('emp-corner/empCornerForm/saveNewCommentLike', async (id, thunkAPI) => {
    const response = await axios.post(api.empCorner.commentdisLikes, { "commentId": id });
    const data = await response.data;
    // if (data.status == "200") {
    //     thunkAPI.dispatch(showMessage({ message: data.message, variant: 'success' }));
