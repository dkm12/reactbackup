...
import { getWorkAnvsryWishes, selectworkWishes } from '../store/workWishes';
...
function EmpCornerListContent(props) {
  ...
  const workAnnivWishes = useSelector(selectworkWishes);
  const [data, setData] = useState(workAnnivWishes);
  const uuid = useSelector(({ auth }) => auth.user.uuid);


  useEffect(() => {
    dispatch(getWorkAnvsryWishes());
  }, [])
  useEffect(() => {
    setData(workAnnivWishes);
    console.log('workAnnivWishes>>>', workAnnivWishes)
  }, [workAnnivWishes]);
        ...
        return (<>
          <Card
            ...
            <CardContent className='pt-5'>
              <Typography component="h4" variant="body1" className={classes.content}>
                {wish.wishComment}
              </Typography>
            </CardContent>
          </Card>
